package keystrokesmod.sToNkS.clickgui.kopagui;
...
public class TabGui extends GuiScreen {
   ...
   /* public TabGui(){
        ...
        for(Module.category cat : Module.category.values()){
            //if(fontRenderer.getStringWidth(cat + "") > maxWidth)
               // maxWidth = fontRenderer.getStringWidth(cat + "");

            CategoryComponent categoryComponent = new CategoryComponent(cat, this.fontRenderer);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        int miniLengt = fontRenderer.getStringWidth(Ravenbplus.version) / 2;
        int nameLengt = fontRenderer.getStringWidth(Ravenbplus.clientName);
        int centerdY = (topBarHeigt - fontRenderer.FONT_HEIGHT)/2;
        GL11.glPushMatrix();
        GL11.glScaled(0.5D, 0.5D, 0.5D);
        drawString(fontRenderer, Ravenbplus.version, (guiX +  margin + topBarHeigt + nameLengt)*2, (guiY + margin + centerdY + fontRenderer.FONT_HEIGHT/2)*2, 0xff464a50);
        GL11.glPopMatrix();

        drawString(fontRenderer, Ravenbplus.clientName, (guiX +  margin + topBarHeigt), (guiY + margin + centerdY), ravenColour);
        //
