...
public class Utils {
   ...
   public static class Player {
      ...
      public static void sendMessageToSelf(String txt) {
         if (isPlayerInGame()) {
            ...
            mc.player.sendMessage(new TextComponentString(txt));
         }
      }
      ...
      public static void aim(Entity en, float ps, boolean pc) {
         if (en != null) {
            ...
            if (t != null) {
               ...
               float p = t[1] + 4.0F + ps;
               if (pc) {
                  mc.getConnection().sendPacket(new CPacketPlayer.Rotation(y, p, mc.player.onGround));
               } else {
      ...
      public static boolean playerUnderBlock() {
         double x = mc.player.posX;
         double y = mc.player.posY + 2.0D;
         double z = mc.player.posZ;
         BlockPos p = new BlockPos(MathHelper.floor(x), MathHelper.floor(y), MathHelper.floor(z));
         return mc.world.isBlockFullCube(p) || mc.world.isBlockNormalCube(p, false);
      }
      ...
      public static int getMaxDamageSlot(){
         ...
         for (int slot = 0; slot <= 8; slot++) {
            ...
            if(itemInSlot == null)
               continue;
            for (AttributeModifier mooommHelp :itemInSlot.getAttributeModifiers(EntityEquipmentSlot.MAINHAND).values()){
      ...
      public static ArrayList<Integer> playerWearingArmor() {
         ...
         for(int armorPiece = 0; armorPiece < 4; armorPiece++){
            if(mc.player.inventory.armorItemInSlot(armorPiece) != null){
               if (armorPiece == 0) {
                  wearingArmor.add(3);
      ...
      public static void fixMovementSpeed(double s, boolean m) {
         if (!m || isMoving()) {
            mc.player.motionX = -Math.sin(correctRotations()) * s;
            mc.player.motionZ = Math.cos(correctRotations()) * s;
         }
      }
      ...
      public static void bop(double s) {
         ...
         if (forward == 0.0D && strafe == 0.0D) {
            ...
            mc.player.motionX = forward * s * cos + strafe * s * sin;
            mc.player.motionZ = forward * s * sin - strafe * s * cos;
         }
      ...
      public static void swing() {
         ...
         int armSwingEnd = p.isPotionActive(MobEffects.HASTE) ? 6 - (1 + p.getActivePotionEffect(MobEffects.HASTE).getAmplifier()) : (p.isPotionActive(MobEffects.WEAKNESS) ? 6 + (1 + p.getActivePotionEffect(MobEffects.WEAKNESS).getAmplifier()) * 2 : 6);
   ...
   public static class Client{
      ...
      public static void setTimerSpeed(float value) {
         try {
            Field timerSpeed = getTimer().getClass().getDeclaredField("tickLength");
            timerSpeed.setFloat(getTimer(), 1000.0F/value);
         }
         catch(Exception ex) {
            System.out.println("koroche timer vinovat :C");
      ...
      public static float getTimerSpeed() {
         try {
            Field timerSpeed = getTimer().getClass().getDeclaredField("tickLength");
            return 1000/timerSpeed.getFloat(getTimer());
         }
         catch(Exception ex) {
            System.out.println("koroche timer vinovat :C");
            System.out.println(ex.getMessage());
            ...
            if (type == 3) {
               ...
               if (type == 4) {
                  ...
                  if (type == 5) {
                     ...
                     if (type == 1) {
                        RenderGlobal.drawSelectionBoundingBox(axis, r, g, b, a);
                     } else if (type == 2) {
                        dbb(axis, r, g, b);
                     }
