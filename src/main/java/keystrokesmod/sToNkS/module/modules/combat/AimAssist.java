...
public class AimAssist extends Module {
   ...
   public static boolean isAFriend(Entity entity) {
      ...
      try {
         ...
         if(Ravenbplus.debugger){
            ...
            Utils.Player.sendMessageToSelf("substring player / " + mc.player.getDisplayName().getUnformattedText().substring(0, 2));
         }
         if(mc.player.isOnSameTeam((EntityLivingBase) entity) || mc.player.getDisplayName().getUnformattedText().startsWith(bruhentity.getDisplayName().getUnformattedText().substring(0, 2))) return true;
   ...
   public Entity getEnemy() {
      ...
      do {
         do {
            ...
            } while((double)mc.player.getDistance(en) > distance.getInput());
   ...
   public static boolean removeFriend(String name) {
      ...
      boolean found = false;
      for (NetworkPlayerInfo networkPlayerInfo : new ArrayList<>(mc.getConnection().getPlayerInfoMap())) {
         Entity entity = mc.world.getPlayerEntityByName(networkPlayerInfo.getDisplayName().getUnformattedText());
         if (entity.getName().equalsIgnoreCase(name) || entity.getCustomNameTag().equalsIgnoreCase(name)) {
