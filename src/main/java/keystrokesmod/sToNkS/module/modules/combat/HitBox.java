...
public class HitBox extends Module {
   ...
   public static void gmo(float partialTicks) {
      if (mc.getRenderViewEntity() != null && mc.world != null) {
         ...
         Vec3d vec6 = null;
         float f1 = 1.0F;
         List list = mc.world.getEntitiesWithinAABBExcludingEntity(mc.getRenderViewEntity(), mc.getRenderViewEntity().getEntityBoundingBox().expand(vec4.x * d0, vec4.y * d0, vec4.z * d0).expand(f1, f1, f1));
         double d3 = d2;
   ...
   private void rh(Entity e, Color c) {
      if (e instanceof EntityLivingBase) {
         ...
         RenderGlobal.drawSelectionBoundingBox(axis, 1f,1f,1f,1f);
         GL11.glEnable(3553);
         GL11.glEnable(2929);
