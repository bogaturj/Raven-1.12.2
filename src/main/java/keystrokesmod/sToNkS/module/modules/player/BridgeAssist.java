...
public class BridgeAssist extends Module {
    ...
    public void onRenderTick(TickEvent.RenderTickEvent e) {
        ...
        if (!(Utils.Player.playerOverAir() && mc.player.onGround)) {
        ...
        if (gliding){
            ...
            if (this.speedYaw > ilovebloat1 || this.speedYaw > ilovebloat2)
                mc.player.rotationYaw = this.waitingForYaw;

            if (this.speedPitch > ilovebloat3 || this.speedPitch > ilovebloat4)
                mc.player.rotationPitch = this.waitingForPitch;

            if (mc.player.rotationYaw < this.waitingForYaw)
                mc.player.rotationYaw += this.speedYaw;

            if (mc.player.rotationYaw > this.waitingForYaw)
                mc.player.rotationYaw -= this.speedYaw;

            if (mc.player.rotationPitch > this.waitingForPitch)
                mc.player.rotationPitch -= this.speedPitch;

            if (mc.player.rotationYaw == this.waitingForYaw && mc.player.rotationPitch == this.waitingForPitch) {
                gliding = false;
                this.waitingForAim = false;
            }
    ...
    public void aimAt(float pitch, float yaw, float fuckedYaw, float fuckedPitch){
       if(setLook.isToggled()) {
               mc.player.rotationPitch = pitch + ((int)fuckedPitch/360) * 360;
