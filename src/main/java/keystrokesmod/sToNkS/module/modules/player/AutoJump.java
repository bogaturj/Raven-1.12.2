...
public class AutoJump extends Module {
   ...
   public void p(PlayerTickEvent e) {
      if (Utils.Player.isPlayerInGame()) {
         if (mc.player.onGround && (!b.isToggled() || !mc.player.isSneaking())) {
            if (mc.world.getCollisionBoxes(mc.player, mc.player.getEntityBoundingBox().offset(mc.player.motionX / 3.0D, -1.0D, mc.player.motionZ / 3.0D)).isEmpty()) {
               this.ju(this.c = true);
            } else if (this.c) {
               this.ju(this.c = false);
            }
