   ...
   public void p(PlayerTickEvent e) {
      ...
      if(doShift.isToggled()) {
         if(lookDown.isToggled()) {
            if(mc.player.rotationPitch < pitchRange.getInputMin() || mc.player.rotationPitch > pitchRange.getInputMax()) {
               shouldBridge = false;
         ...
         if (mc.player.onGround) {
            ...
            else if (mc.player.isSneaking() && !Keyboard.isKeyDown(mc.gameSettings.keyBindSneak.getKeyCode()) && onHold.isToggled()) {
               isShifting = false;
               shouldBridge = false;
               this.setShift(false);
            }
            ...
            else if(mc.player.isSneaking() && !onHold.isToggled()) {
               isShifting = false;
               this.setShift(false);
               shouldBridge = true;
            }
         ...
         if (mc.currentScreen == null) {
            if(shouldBridge) {
               ...
               if(BlockAmountInfo.values()[(int)blockShowMode.getInput() - 1] == BlockAmountInfo.BLOCKS_IN_CURRENT_STACK) {
                  totalBlocks = Utils.Player.getBlockAmountInCurrentStack(mc.player.inventory.currentItem);
               } else {
