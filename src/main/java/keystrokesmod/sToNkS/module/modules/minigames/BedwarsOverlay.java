...
import static keystrokesmod.sToNkS.utils.Utils.Java.round;
...
public class BedwarsOverlay extends Module {
    ...
    public void onChatMessageRecieved(ClientChatReceivedEvent event) {
        if (Utils.Player.isPlayerInGame()) {
            if (Utils.Java.str(event.getMessage().getUnformattedText()).startsWith("Sending you to")) {
                playerStats.clear();
            }
        }
    }
    ...
    private void drawStats(NetworkPlayerInfo player, FontRenderer fr) {
        ...
        if (Utils.URLS.hypixelApiKey.isEmpty()){
            ...
            if(!playerStats.containsKey(UUID)){
                Ravenbplus.getExecutor().execute(() -> getBedwarsStats(UUID));
