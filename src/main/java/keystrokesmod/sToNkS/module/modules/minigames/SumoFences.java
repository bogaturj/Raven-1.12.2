...
public class SumoFences extends Module {
   ...
   public void onDisable() {
      ...
      for (BlockPos p : f_p) {
         for (int i = 0; (double) i < b.getInput(); ++i) {
            ...
            if (mc.world.getBlockState(p2).getBlock() == this.f) {
   ...
   public void m(MouseEvent e) {
      if (e.isButtonstate() && (e.getButton() == 0 || e.getButton() == 1) && Utils.Player.isPlayerInGame() && this.is()) {
            ...
            if (SumoFences.this.is()) {
               ...
               for (BlockPos p : SumoFences.f_p) {
                  for (int i = 0; (double) i < SumoFences.b.getInput(); ++i) {
                     BlockPos p2 = new BlockPos(p.getX(), p.getY() + i, p.getZ());
                     if (Module.mc.world.getBlockState(p2).getBlock() == Blocks.AIR) {
                        Module.mc.world.setBlockState(p2, SumoFences.this.f);
                     }
                  }
               }
